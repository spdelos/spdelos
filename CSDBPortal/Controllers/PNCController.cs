...
namespace CSDBPortal.Controllers
{
    public class PNCController : BaseController
    {
        ...
        public async Task<JsonResult> GetPNCs(
            ...
            string? seg5, string? seg6, string? seg7)
        {
            ...
            var rows = await q.OrderBy(p => p.FullPNC)
                              .Select(p => new {
                                  ...
                                  p.CreatedBy, CreatedOn = p.CreatedOn.ToString("yyyy-MM-dd HH:mm")
                              })
                              .ToListAsync();
        ...
        [HttpPost]
        public async Task<JsonResult> SavePNC(
            [FromBody] PartNumberCode pnc)
        {
            if (pnc == null)
        ...
        public IActionResult DownloadTemplate()
        {
            ...
            for (int r = 0; r < eqData.GetLength(0); r++)
            {
                wsEq.Cell(r + 2, 1).Value = eqData[r, 0];
                wsEq.Cell(r + 2, 2).Value = eqData[r, 1];
            }

            // Ref sheet – Module Codes
            ...
            for (int r = 0; r < modData.GetLength(0); r++)
            {
                wsMod.Cell(r + 2, 1).Value = modData[r, 0];
                wsMod.Cell(r + 2, 2).Value = modData[r, 1];
            }
        ...
        {
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.ModelId,    @"^[A-Z]{2,4}[0-9]?$")) return $"ModelId '{p.ModelId}' is invalid (2-4 letters, optional trailing digit).";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.EqCode,     @"^(70|71|72|73|74|75|76|77|78|79|80)$")) return $"EqCode '{p.EqCode}' is invalid (must be 70-80).";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.ModCode,    @"^(EGN|EEX|LPC|COU|FAN|ICA|HPC|DCO|TNZ|HPT|LPT|TEC|MGB|AGB)$")) return $"ModCode '{p.ModCode}' is not a recognised module code.";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.SubAsmCode, @"^[A-Z]{3}$"))          return $"SubAsmCode '{p.SubAsmCode}' is invalid (exactly 3 letters).";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.SeqDigits,  @"^[0-9]{5}$"))          return $"SeqDigits '{p.SeqDigits}' is invalid (exactly 5 digits).";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.MaintLevel, @"^[OID]$"))             return $"MaintLevel '{p.MaintLevel}' is invalid (O, I or D).";
            if (!System.Text.RegularExpressions.Regex.IsMatch(p.RevSuffix,  @"^[A-Z]$"))             return $"RevSuffix '{p.RevSuffix}' is invalid (single letter A-Z).";
            return null;
        }
    }
}
